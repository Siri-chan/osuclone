...
public class osu_clone extends PApplet {
...
int pink = color(255, 192, 203); //osu!clone pink
public void setup() {
  //song = new SoundFile(this, "Grey Beard Halt.mp3");
   //sets forced window size
  ...
  surface.setTitle("osu!clone: " + splashtext[PApplet.parseInt(random(11))]); //sets the funny splash text !!
  /* !!IMPORTANT DEV NOTE!!
...
public void draw() {
  ...
  if (loli == false) { //if the menus are open
    if (loli2 == true) { //checks for a player failure state
      ...
      textSize(100); 
      if (mouseX > 250 && mouseX < 550 && mouseY > 82.5f && mouseY < 467.5f) { //draws the playbutton if the mouse is close enough to it
      ...
      if (index < lines.length) {
        ...
        if (die == false) {
          ...
          try { //loads data from the file
            circlex[circle] = PApplet.parseInt(pieces[piece]);
            piece++;
            circley[circle] = PApplet.parseInt(pieces[piece]);
            piece++;
            circle++;
          } 
...
class OtherSketch extends PApplet {
  ...
  int pink = color(255, 192, 203); //osu!clone pink
  public void setup() {
    surface.setTitle("osu!clone editor: " + splashtext[PApplet.parseInt(random(10))]); //sets the funny splash text !!
  ...
  public void draw() {  //mostly just filling stuff, totally normal
    noFill();
    stroke(0);
    strokeWeight(5);
    rect(0, 0, 800, 600);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "osu_clone" };
    if (passedArgs != null) {
